import { RoleAccess } from "@prisma/client";

export const auths = [
  {
    full_name: "Lucas Cabral",
    email: "lucas23@email.com",
    password: "Doctor_123456",
    role: RoleAccess.ADMIN,
  },
  {
    full_name: "Jazmin Orellanada",
    email: "jaz_vega@email.com",
    password: "Doctor_123456",
    role: RoleAccess.DOCTOR,
  },
  {
    full_name: "Micaela Sanchez",
    email: "mica_sanchez@email.com",
    password: "Doctor_123456",
    role: RoleAccess.DOCTOR,
  },
  {
    full_name: "Carlos Gómez",
    email: "carlos.gomez@email.com",
    password: "Doctor_123456",
    role: RoleAccess.DOCTOR,
  },
  {
    full_name: "Ana Martínez",
    email: "ana.martinez@email.com",
    password: "Doctor_123456",
    role: RoleAccess.DOCTOR,
  },
  {
    full_name: "Pedro González",
    email: "pedro.gonzalez@email.com",
    password: "Doctor_123456",
    role: RoleAccess.DOCTOR,
  },
  {
    full_name: "Laura Fernández",
    email: "laura.fernandez@email.com",
    password: "Doctor_123456",
    role: RoleAccess.DOCTOR,
  },
  {
    full_name: "José Pérez",
    email: "jose.perez@email.com",
    password: "Doctor_123456",
    role: RoleAccess.DOCTOR,
  },
  {
    full_name: "María López",
    email: "maria.lopez@email.com",
    password: "Doctor_123456",
    role: RoleAccess.DOCTOR,
  },
  {
    full_name: "Fernando Díaz",
    email: "fernando.diaz@email.com",
    password: "Doctor_123456",
    role: RoleAccess.DOCTOR,
  },
  {
    full_name: "Elena Rodríguez",
    email: "elena.rodriguez@email.com",
    password: "Doctor_123456",
    role: RoleAccess.DOCTOR,
  },
  {
    full_name: "Valentina Ruiz",
    email: "valentina.ruiz@email.com",
    password: "Doctor_123456",
    role: RoleAccess.DOCTOR,
  },
  {
    full_name: "Santiago Molina",
    email: "santiago.molina@email.com",
    password: "Doctor_123456",
    role: RoleAccess.DOCTOR,
  },
  {
    full_name: "Martina Castro",
    email: "martina.castro@email.com",
    password: "Doctor_123456",
    role: RoleAccess.DOCTOR,
  },
  {
    full_name: "Agustín Herrera",
    email: "agustin.herrera@email.com",
    password: "Doctor_123456",
    role: RoleAccess.DOCTOR,
  },
  {
    full_name: "Camila Torres",
    email: "camila.torres@email.com",
    password: "Doctor_123456",
    role: RoleAccess.DOCTOR,
  },
  {
    full_name: "Franco Medina",
    email: "franco.medina@email.com",
    password: "Doctor_123456",
    role: RoleAccess.DOCTOR,
  },
  {
    full_name: "Sofía Romero",
    email: "sofia.romero@email.com",
    password: "Doctor_123456",
    role: RoleAccess.DOCTOR,
  },
  {
    full_name: "Tomás Villalba",
    email: "tomas.villalba@email.com",
    password: "Doctor_123456",
    role: RoleAccess.DOCTOR,
  },
  {
    full_name: "Lucía Navarro",
    email: "lucia.navarro@email.com",
    password: "Doctor_123456",
    role: RoleAccess.DOCTOR,
  },
  {
    full_name: "Emilia Paredes",
    email: "emilia.paredes@email.com",
    password: "Doctor_123456",
    role: RoleAccess.ADMIN,
  },
];
